...
def test_iter_supported_mime_types_includes_registered_handlers():
    ...
    assert {"image/jpeg", "image/png", "image/bmp", "application/pdf"} <= set(
        mime_types
    )


def test_disarm_file_missing_file_raises(tmp_path: Path):
...
def test_disarm_file_dispatches_on_content_not_extension(tmp_path: Path, make_image):
...
def test_disarm_folder_in_place(tmp_path: Path, make_image):
