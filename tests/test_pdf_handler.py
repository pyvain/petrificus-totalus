...
def test_pdf_roundtrip_preserves_page_colors(tmp_path: Path, make_pdf):
    src = make_pdf(tmp_path / "colors.pdf", page_colors=((1, 0, 0), (0, 1, 0), (0, 0, 1)))
...
def test_pdf_petrify_rewrites_bytes(tmp_path: Path, make_pdf):
...
def test_pdf_petrify_adds_searchable_ocr_text_layer(tmp_path: Path, make_pdf):
    src = make_pdf(
        tmp_path / "scanned.pdf", page_colors=((1, 1, 1),), text="Hello OCR World", size=(400, 300)
    )
    ...
    with pymupdf.open(src) as before:
        ...
        assert before[0].get_text().strip() == "Hello OCR World"

    petrify_file(src)

    with pymupdf.open(src) as after:
        assert after[0].get_text().strip() == "Hello OCR World"
...
def test_detect_languages_keeps_english_as_secondary_for_non_english_primary():
    with pymupdf.open() as doc:
        ...
        page.insert_text(
            (20, 150), "Ceci est un texte suffisamment long en francais pour la detection", fontsize=14
        )
...
def test_detect_languages_does_not_duplicate_english_primary():
    with pymupdf.open() as doc:
        page = doc.new_page(width=400, height=300)
        page.insert_text(
            (20, 150), "This is plainly a long enough sentence of English text", fontsize=14
        )
...
@_requires_dejavu_sans
def test_pdf_petrify_recognizes_mixed_russian_and_english_text(tmp_path: Path):
    ...
    with pymupdf.open() as doc:
        ...
        page.insert_text(
            (20, 150), text, fontsize=20, color=(0, 0, 0), fontfile=str(_DEJAVU_SANS), fontname="F0"
        )
    ...
    with pymupdf.open(src) as after:
        recovered = after[0].get_text().strip()

    assert recovered == text


def test_pdf_petrify_recognizes_non_english_text_via_detected_language(tmp_path: Path, make_pdf):
    ...
    src = make_pdf(tmp_path / "french.pdf", page_colors=((1, 1, 1),), text=text, size=(400, 300))
...
def test_petrify_file_writes_pdf_to_explicit_output_path(tmp_path: Path, make_pdf):
